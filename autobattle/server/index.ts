...
import { RoomState, PlayerState, PlacedUnit, Phase } from './sim/types.js';
...
const connections = new Map<string, { ws: WebSocket; roomCode: string; playerId: string }>();
...
function sendToPlayer(playerId: string, roomCode: string, message: any): void {
  for (const [wsId, conn] of connections.entries()) {
...
function getRoomState(room: RoomState): any {
...
function handlePhaseTransition(room: RoomState): void {
...
function runCombat(room: RoomState): void {
  const players = Array.from(room.players.values());
...
function handleMessage(ws: WebSocket, wsId: string, message: any): void {
  const { type } = message;
...
function handlePlayerAction(room: RoomState, player: PlayerState, message: any): void {
  const { type } = message;
